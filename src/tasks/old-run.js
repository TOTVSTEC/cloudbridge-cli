var AppTask = cb_require('tasks/app-task'),
	...
	CloudBridgeCore = require(__basedir + '/core'),
	CloudBridgeProject = require('./project'),
	CloudBridgeResources = CloudBridgeCore.resources,
	ConfigJson = CloudBridgeCore.configJson,
	ConfigXml = CloudBridgeCore.configXml,
	Cordova = CloudBridgeCore.cordova,
	Hooks = CloudBridgeCore.hooks,
	Serve = CloudBridgeCore.serve,
	State = CloudBridgeCore.state,
	Utils = CloudBridgeCore.utils;

var OldRunTask = function() { };
OldRunTask.prototype = new AppTask();

OldRunTask.prototype.run = function(cloudbridge, argv) {
	...
	try {
		...
		return q.then(function(serveOptions) {
				...
				if (addDefaultResources && self.addIcons) {
					return CloudBridgeResources.copyIconFilesIntoResources(appDirectory)
						.then(function() {
							return CloudBridgeResources.addCloudBridgeIcons(appDirectory, argv._[2]);
						});
				}
				else {
					return false;
...
OldRunTask.prototype.runCordova = function(cmdName, argv) {
...
OldRunTask.prototype.runCordova1 = function(cmdName, argv) {
	...
	if (cmdName == 'platform' && cmdArgs.length > 0) {
		// `cloudbridge platform <PLATFORM>` used to actually run `cloudbridge platform add <PLATFORM>`
	...
	if (self.isLiveReload) {
		cordovaProcess.on('exit', function() {
			...
			setTimeout(function() {
				...
				ConfigXml.setConfigJson(process.cwd(), {
					resetContent: true,
		...
		process.on('exit', function() {
			...
			ConfigJson.setConfigJson(process.cwd(), {
				resetContent: true,
				errorWhenNotFound: false
			});
		});
...
OldRunTask.prototype.setupLiveReload = function(argv) {
	var d = Q.defer();
	...
	try {
		project = CloudBridgeProject.load(process.cwd());
	}
	catch (ex) {
	...
	options.appDirectory = process.cwd();//called from cli - cloudbridge serve - cwd is this
	...
	return promise
		...
		.catch(function(error) {
			console.log('There was an error serving your CloudBridge application for run', error);
			console.log(error.stack);
			throw error;
		});
};

module.exports = OldRunTask;
