...
var Settings = {
	apiUrl: 'http://res.cloudbridge.io',
...
function generate(dir, options) {
	...
	if (options.default) {
		if (hasExistingResources(appDirectory) && !options.force) {
			...
			logging.logger.info(('Running with the force flag will overwrite your resources directory and modify your ' + Settings.configFile + ' file').red.bold);
			return Q.reject('RESOURCES_EXISTS');
		}

		return copyIconFilesIntoResources(appDirectory, options.force)
			.then(function() {
				return addCloudBridgeIcons(appDirectory, 'ios');
			})
			.then(function() {
				return addCloudBridgeIcons(appDirectory, 'android');
			});
	}
	...
	if (!hasPlatforms) {
		...
		logging.logger.error('Please add a platform, for example: cloudbridge platform add ios');
		logging.logger.error('Or provide a platform name, for example: cloudbridge resources android');
	...
	return getConfigData(appDirectory).then(function() {
		...
		if (options.icon || options.i) {
			logging.logger.info('CloudBridge icon resources generator');
			promises.push(queueResTypeImages('icon'));
		}
		else if (options.splash || options.s) {
			logging.logger.info('CloudBridge splash screen resources generator');
			promises.push(queueResTypeImages('splash'));
		}
		else {
			logging.logger.info('CloudBridge icon and splash screen resources generator');
			promises.push(queueResTypeImages('icon'));
			promises.push(queueResTypeImages('splash'));
		}
...
function updateConfigData() {
	...
	function updateImageNode(image) {
		...
		if (!platformConfigData) {
			configData.platform.push({ '$': { name: image.platform } });
	...
	function buildDefaultIconNode() {
		...
		if (defaultIcon) {
			configData.icon = [{ '$': { src: defaultIcon.src } }];
			madeChanges = true;
		}
	}
	...
	function buildPreferenceSplashNodes() {
		...
		if (hasAndroidSplash) {
			if (!configData.preference) {
				configData.preference = [];
			}

			var hasScreenPref = _.find(configData.preference, function(d) {
				return d && d.$ && d.$.name == 'SplashScreen';
			});

			if (!hasScreenPref) {
				configData.preference.push({ '$': { name: 'SplashScreen', value: 'screen' } });
				madeChanges = true;
			}

			var hasDelayPref = _.find(configData.preference, function(d) {
				return d && d.$ && d.$.name == 'SplashScreenDelay';
			});

			if (!hasDelayPref) {
				configData.preference.push({ '$': { name: 'SplashScreenDelay', value: '3000' } });
				madeChanges = true;
			}
		}
...
function getOrientationConfigData(platform) {
	if (configData && configData.preference) {
		var n = _.find(configData.preference, function(d) {
...
function getPlatformConfigData(platform) {
	if (configData && configData.platform) {
		return _.find(configData.platform, function(d) {
...
function writeConfigData(appDirectory, configData) {
	...
	fs.writeFile(path.join(appDirectory, Settings.configFile), jsonString, function(err) {
		if (err) {
...
function addCloudBridgeIcons(appDirectory, platform, forceAddResources) {
	...
	return promise
		...
		.then(function(data) {
			if (!data.platform || (data.platform.length == 1 && data.platform[platform] !== undefined)) {
...
function copyIconFilesIntoResources(appDirectory, forceAddResources) {
	...
	var cloudbridgeResourcesUrl = 'https://github.com/driftyco/ionic-default-resources/archive/master.zip';
	logging.logger.debug('uzip to: ', unzipPath);
	logging.logger.info('Downloading Default CloudBridge Resources'.yellow);

	return Utils.fetchArchive(unzipPath, cloudbridgeResourcesUrl)
		.then(function() {
			shelljs.config.silent = true;
			shelljs.mv('resources/cloudbridge-default-resources-master/*', 'resources');
			shelljs.rm('-rf', 'resources/cloudbridge-default-resources-master');
			logging.logger.info('Done adding default CloudBridge resources'.yellow);
		});
}
...
function addPlatformIcons(platform, configData) {
	...
	var platElem = configData.platform[platform] || {};

	configData.platform[platform] = platElem;
...
function addSplashScreenPreferences(configData) {
	...
	var preferences = configData.preference;
	...
	configData.preference.forEach(function(pref) {
...
module.exports = {
	...
	Platforms: Platforms,
	generate: generate,
	addCloudBridgeIcons: addCloudBridgeIcons,
